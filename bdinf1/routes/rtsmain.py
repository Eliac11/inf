    ...
    return [
        fastUIcomponents.Page(  # Page provides a basic container for components
            components=[
                ...
                #     ],
                # ),
                fastUIcomponents.Heading(text='Главная', level=2),
                fastUIcomponents.Table(
                    data=forms,
                    columns=[
                        DisplayLookup(field='name', on_click=GoToEvent(
                ...
                fastUIcomponents.Div(
                    ...
                    components=[
                        fastUIcomponents.Heading(text='Отчет', level=2),
                        fastUIcomponents.Button(text="Отчет счета", on_click=GoToEvent(url='/service/downloadFile?reptype=allAcc', target='_blank')),
                        fastUIcomponents.Button(text="Отчет закрытые счета", class_name='+ ms-4', on_click=GoToEvent(url='/service/downloadFile?reptype=endAcc', target='_blank')),
