...
def users_table(account_id: int, db: Session = Depends(get_db)) -> list[AnyComponent]:
    acc = db.query(tblAccount).filter(
        tblAccount.intAccountId == account_id).first()

    queryoperations = list(db.query(tblOperation).filter(
        tblOperation.intAccountId == acc.intAccountId))
    if queryoperations:
        operations = parse_obj_as(List[pdtOperation], db.query(
            tblOperation).filter(tblOperation.intAccountId == acc.intAccountId))
        tableview = fastUIcomponents.Div(class_name='border-top mt-3 pt-1',
                                         components=[
                                             fastUIcomponents.Heading(
                                                 text="Операции", level=3),
                                             fastUIcomponents.Table(
                                                 data=operations,
                                                 columns=[
                                                     DisplayLookup(
                                                         field='datOperation', title="Дата орепации"),
                                                     DisplayLookup(
                                                         field='operation_type', title="Тип"),
                                                     DisplayLookup(field='fltValue', title="Сумма", on_click=GoToEvent(
                                                         url="/forms/delOperation?intOperationId={intOperationId}"))
                                                 ]
                                             )
                                         ]
                                         )
    ...
    cardInfo = fastUIcomponents.Div(
        ...
        components=[
            fastUIcomponents.Heading(
                text=f"ID:              {acc.intAccountId}", level=4),
            fastUIcomponents.Heading(
                text=f"Тип:            {acctype}", level=4),
            fastUIcomponents.Heading(text=f"ФИО: {clientfullname}", level=4),
            fastUIcomponents.Heading(
                text=f"Дата открытия:    {acc.datAccountBegin}", level=4),
            fastUIcomponents.Heading(
                text=f"Дата закрытия:      {acc.datAccountEnd}", level=4),
            fastUIcomponents.Heading(
                text=f"Номер счета:   {acc.txtAccountNumber}", level=4),
            fastUIcomponents.Heading(
                text=f"Баланс:      {acc.fltAccountSum}", level=4),
        ]
    )
    ...
    return [
        fastUIcomponents.Page(  # Page provides a basic container for components
            components=[
                ...
                cardInfo,
                tableview,
                fastUIcomponents.Button(
                    text='Добавиить новую операцию', on_click=PageEvent(name='open-form')),
                fastUIcomponents.Modal(
                    ...
                    body=[
                        ...
                        fastUIcomponents.ModelForm(model=formNewOperation.SelectForm, display_mode='default',
                                                   submit_url=f"/api/forms/formNewOperation?account_id={account_id}", submit_trigger=PageEvent(name='submit-form')),
                    ],
...
async def big_form_post(account_id: int, form: Annotated[formNewOperation.SelectForm, fastui_form(formNewOperation.SelectForm)], db: Session = Depends(get_db)):
    ...
    sql_query = text(
        "INSERT INTO tblOperation (intOperationTypeId, intAccountId, fltValue, datOperation) VALUES (:type_id, :account_id, :value, :operation_date)")
...
async def big_form_post(intOperationId: int, db: Session = Depends(get_db)):
    ...
    operation = db.query(tblOperation).filter(
        tblOperation.intOperationId == intOperationId).first()
